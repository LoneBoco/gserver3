...
#			if defined(UNICODE)
			{
				(void)lstrcpynW(szFile, pNotify->FileName,
					std::min<DWORD>(static_cast<DWORD>(MAX_PATH), pNotify->FileNameLength / sizeof(WCHAR) + 1)
				);
			}
#			else
			{
...
#			endif
			...
			if (pWatch->callback != nullptr)
				pWatch->callback(pWatch->watch_id, pWatch->dir, std::filesystem::path{ szFile }, translateAction(pNotify->Action));
		}
